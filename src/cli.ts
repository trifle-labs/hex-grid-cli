...
Options:
  ...
  --style, -s         Visual style: default, minimal (default: default)
  --demo              Run demo with example patterns

Examples:
  hex-grid-cli --demo
  hex-grid-cli --type small-pointy --demo
  hex-grid-cli --type large-flat --style minimal --demo
`);
}
...
function runDemo(type: string, style: VisualStyle) {
  const printer = getPrinter(type);
  
  console.log(`\n=== Demo: ${type} (${style} style) ===\n`);
  
  if (type.includes('pointy')) {
    // Pointy demo
    const board = new AsciiBoard(0, 2, 0, 0, printer, style);
    ...
    // Flat demo
    const board = new AsciiBoard(0, 2, 0, 1, printer, style);
...
function main() {
  ...
  let visualStyle: VisualStyle = 'default';
  let demo = false;
  ...
  for (let i = 0; i < args.length; i++) {
    if ((args[i] === '--type' || args[i] === '-t') && i + 1 < args.length) {
      hexType = args[i + 1];
      i++;
    } else if ((args[i] === '--style' || args[i] === '-s') && i + 1 < args.length) {
      const styleArg = args[i + 1];
      if (styleArg !== 'default' && styleArg !== 'minimal') {
        console.error('Error: style must be "default" or "minimal"');
        process.exit(1);
      }
      visualStyle = styleArg;
      i++;
    } else if (args[i] === '--demo') {
      demo = true;
    }
